...
from src.data.image_datasets import CIFAR10Dataset

parser = argparse.ArgumentParser(
    description='Trains a CIFAR Classifier',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('--data-dir', type=str, default="data/processed/cifar")
parser.add_argument(
    '--model',
    '-m',
    type=str,
...
parser.add_argument(
    ...
    '-na',
    action='store_true',
    help='Turn off AugMix augmentations (standard model).')
parser.add_argument(
    '--all-ops',
    '-all',
    action='store_true',
...
class AugMixDataset(torch.utils.data.Dataset):
  ...
  def __init__(self, dataset, preprocess, no_jsd=False, no_aug=False):
...
def main():
  ...
  if args.no_aug:
      args.no_jsd = True
  train_data = AugMixDataset(train_data, preprocess, args.no_jsd, args.no_aug)
  ...
  if args.evaluate:
    ...
    test_c_acc = test_c(net, test_data, args.data_dir)
  ...
  log_path = os.path.join(args.save, args.model + '_training_log.csv')
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    print(
        'Epoch {0:3d} | Time {1:5d} | Train Loss {2:.4f} | Val Loss {3:.3f} |'
        ' Val Error {4:.2f}'
        .format((epoch + 1), int(time.time() - begin_time), train_loss_ema,
                val_loss, 100 - 100. * val_acc))

  test_c_acc = test_c(net, test_data, os.path.join(args.data_dir, "CIFAR-10-C"))
  print('Mean Corruption Error: {:.3f}'.format(100 - 100. * test_c_acc))
